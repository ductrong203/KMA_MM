...
//     useEffect(() => {
//         if (heDaoTaoId) fetchKhoaDaoTaoList(heDaoTaoId);
//     }, [heDaoTaoId]);
...
//     const handleDotHocChange = (event) => setDotHoc(event.target.value);
...
//             setIsLoading(true);
//             const thoiKhoaBieuData = {
//                 ky_hoc: kyHoc, dot_hoc: dotHoc, lop_id: lopId, mon_hoc_id: monHocId,
//                 giang_vien_id: giangVienId, giang_vien: giangVien, phong_hoc: phongHoc,
//                 tiet_hoc: tietHoc, trang_thai: trangThai
//             };
//             try {
//                 if (editId !== null) {
//                     await updateThoiKhoaBieu(editId, thoiKhoaBieuData);
...
//     const resetForm = () => {
//         setKyHoc(2025); setDotHoc(1); setLopId(""); setMonHocId(""); setGiangVienId("");
//         setGiangVien(""); setPhongHoc(""); setTietHoc(""); setTrangThai(1);
//         setLopSearch(""); setMonHocSearch(""); setGiangVienSearch(""); setEditIndex(null);
//         setEditId(null); setOpen(false);
//     };

//     const handleEdit = (tkb, index) => {
//         setKyHoc(tkb.ky_hoc || 2025); setDotHoc(tkb.dot_hoc || 1); setLopId(tkb.lop_id);
//         setMonHocId(tkb.mon_hoc_id); setGiangVienId(tkb.giang_vien_id); setGiangVien(tkb.giang_vien || "");
//         setPhongHoc(tkb.phong_hoc || ""); setTietHoc(tkb.tiet_hoc || "");
//         setTrangThai(tkb.trang_thai !== undefined ? tkb.trang_thai : 1);
//         setEditIndex(index); setEditId(tkb.id); setOpen(true);
//     };
...
//     const findMonHocNameInView = (id) => monHocFilList.find(mon => mon.id === id)?.ten_mon_hoc || "Không tìm thấy";
...
//                     {/* Bộ lọc - Always visible */}
//                     <Box sx={{ p: 2, borderRadius: 1, mx: "auto", mt: 3, border: '1px solid #e0e0e0' }}>
//                         {/* <Typography
//                             variant="h6"
//                             fontWeight="bold"
//                             textAlign="center"
...
//                                     <InputLabel sx={{ fontSize: '0.9rem' }}>Hệ đào tạo</InputLabel>
//                                     <Select
...
//                             <Grid item xs={12} sm={3}>
//                                 <FormControl fullWidth size="small" variant="outlined">
//                                     <InputLabel sx={{ fontSize: '0.9rem' }}>Khóa đào tạo</InputLabel>
//                                     <Select
...
//                             <Grid item xs={12} sm={3}>
//                                 <FormControl fullWidth size="small" variant="outlined">
//                                     <InputLabel sx={{ fontSize: '0.9rem' }}>Kỳ học</InputLabel>
//                                     <Select
//                                         value={kyHocFilter}
...
//                             <Grid item xs={12} sm={3}>
//                                 <FormControl fullWidth size="small" variant="outlined">
//                                     <InputLabel sx={{ fontSize: '0.9rem' }}>Lớp</InputLabel>
//                                     <Select
//                                         value={lopIdFilter}
...
//                                         textTransform: 'none',
//                                         fontWeight: 'bold',
//                                         backgroundColor: kyHocFilter || lopIdFilter || heDaoTaoFilter || khoaDaoTaoFilter ? "#d32f2f" : "#1976d2",
//                                         "&:hover": {
//                                             backgroundColor: kyHocFilter || lopIdFilter || heDaoTaoFilter || khoaDaoTaoFilter ? "#b71c1c" : "#115293"
//                                         }
//                                     }}
//                                     onClick={() => {
//                                         if (kyHocFilter || lopIdFilter || heDaoTaoFilter || khoaDaoTaoFilter) {
...
//                                     }}
//                                 >
//                                     {kyHocFilter || lopIdFilter || heDaoTaoFilter || khoaDaoTaoFilter ? "Hủy Bộ Lọc" : "Áp Dụng"}
//                                 </Button>
//                             </Grid>
//                         </Grid>
//                     </Box>
...
//                                 getOptionLabel={(option) => option.ho_ten}
//                                 value={filteredGiangVienList.find((g) => g.id === giangVienId) || null}
//                                 onChange={(event, newValue) => handleGiangVienChange({ target: { value: newValue ? newValue.id : null } })}
...
//     const resetForm = () => {
//         // Pre-fill form with filter values if they exist, otherwise use defaults
//         setKyHoc(kyHocFilter || 2025);
//         setDotHoc(1); // No filter equivalent, so reset to default
...
//         // If khoaDaoTaoFilter exists, update kyHocOptions based on it
//         if (khoaDaoTaoFilter) {
//             const selectedKhoa = khoaDaoTaoList.find(khoa => khoa.id === khoaDaoTaoFilter);
//             if (selectedKhoa) {
//                 let years = [];
//                 if (selectedKhoa.nam_hoc.includes("-")) {
//                     const [start, end] = selectedKhoa.nam_hoc.trim().split("-").map(Number);
...
//                             startIcon={<AddIcon />}
//                             onClick={() => {
//                                 resetForm(); // Reset form with filter values
...
const ThoiKhoaBieu = () => {
    ...
    const [monHocFilList, setMonHocFilList] = useState([]);
    const [khoaDaoTaoList, setKhoaDaoTaoList] = useState([]);
    // Thêm state mới cho danh sách khóa đào tạo trong bộ lọc
    const [khoaDaoTaoFilterList, setKhoaDaoTaoFilterList] = useState([]);
    ...
    const fetchKhoaDaoTaoList = async (heDaoTaoId) => {
        try {
            const response = await getDanhSachKhoaDaoTaobyId(heDaoTaoId);
            return response; // Trả về dữ liệu để sử dụng trong các hàm khác
        } catch (error) {
            console.error("Lỗi khi lấy danh sách khóa đào tạo:", error);
            return [];
        }
    ...
    // Cập nhật danh sách khóa đào tạo trong bộ lọc khi heDaoTaoFilter thay đổi
    useEffect(() => {
        if (heDaoTaoFilter) {
            fetchKhoaDaoTaoList(heDaoTaoFilter).then((response) => {
                setKhoaDaoTaoFilterList(response);
                // Reset khoaDaoTaoFilter nếu nó không còn hợp lệ với heDaoTaoFilter mới
                if (!response.some(khoa => khoa.id === khoaDaoTaoFilter)) {
                    setKhoaDaoTaoFilter("");
                }
            });
        } else {
            setKhoaDaoTaoFilterList([]);
            setKhoaDaoTaoFilter("");
        }
    }, [heDaoTaoFilter]);

    // Cập nhật danh sách khóa đào tạo trong form thêm/sửa khi heDaoTaoId thay đổi
    useEffect(() => {
        if (heDaoTaoId) {
            fetchKhoaDaoTaoList(heDaoTaoId).then((response) => {
                setKhoaDaoTao(response);
                // Reset khoaDaoTaoId nếu nó không còn hợp lệ với heDaoTaoId mới
                if (!response.some(khoa => khoa.id === khoaDaoTaoId)) {
                    setKhoaDaoTaoId("");
                }
            });
        } else {
    ...
    const fetchHeDaoTaoList = async () => {
        try {
            ...
            console.error("err fetching he dao tao list", error);
        }
    };
                            ...
                            <Grid item xs={12} sm={3}>
                                <FormControl fullWidth size="small" variant="outlined">
                                    ...
                                    <Select
                                        ...
                                        onChange={(e) => setKhoaDaoTaoFilter(e.target.value)}
                                        label="Khóa đào tạo"
                                        disabled={!heDaoTaoFilter} // Vô hiệu hóa nếu chưa chọn hệ đào tạo
                                    >
                                        <MenuItem value="">Tất cả</MenuItem>
                                        {khoaDaoTaoFilterList.map((item) => (
                                            <MenuItem key={item.id} value={item.id}>{item.ten_khoa}</MenuItem>
                                        ))}
                                    </Select>
                                </FormControl>
