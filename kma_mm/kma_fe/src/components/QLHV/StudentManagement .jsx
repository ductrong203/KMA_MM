...
const StudentManagement = () => {
  ...
  const generateMaSinhVien = (lop_id = lop_tu_sinh?.lop_id) => {
    ...
    const soLuongSinhVien = students.filter(
      (sv) => sv.lop_id === lop_id
    ).length;
  ...
  const filteredStudents = students.filter((student) => {
    ...
    const matchesLop = lop_tu_sinh.lop_id
      ? student.lop_id === lop_tu_sinh.lop_id
      : true;
  ...
  const handleSave = async () => {
    try {
      ...
      if (editIndex === null) {
        ...
        res = await updateStudentById(
          formattedStudentData,
          formattedStudentData.id
        );
  ...
  const handleSaveMilitary = async () => {
    try {
      ...
      if (editIndex !== null) {
        await updateMilitaryInfoByStudentId(
  ...
  return (
    <Container maxWidth="xl">
      ...
      <Grid container spacing={2} alignItems="center" sx={{ marginTop: "4px" }}>
        ...
        <Grid item xs={4}>
          <FormControl fullWidth>
            <InputLabel sx={{ padding: "0 2px", backgroundColor: "white" }}>
              Lớp
            </InputLabel>
            <Select
            ...
            {paginatedStudents.map((student, index) => (
              <TableRow key={student.id}>
                ...
                <TableCell>
                  <Button
                    variant="outlined"
                    onClick={() => handleOpenDetail(student?.id - 1)}
                  >
                    Xem chi tiết
                  </Button>
                  <Button
            ...
            <Grid container spacing={2}>
              {[
                ...
                { label: "Ngày vào đảng", value: studentData.ngay_vao_dang },
                {
                  ...
                  value: studentData.ngay_vao_truong,
                },
                ...
                {
                  label: "Trúng tuyển theo nguyện vọng",
                  value: studentData.trung_tuyen_theo_nguyen_vong,
                },
                {
                  label: "Năm tốt nghiệp PTTH",
                  value: studentData.nam_tot_nghiep_PTTH,
                },
                {
                  label: "Thành phần gia đình",
                  value: studentData.thanh_phan_gia_dinh,
                },
                {
                  label: "Đối tượng đào tạo",
                  value: studentData.doi_tuong_dao_tao,
                },
                {
                  label: "DV liên kết đào tạo",
                  value: studentData.dv_lien_ket_dao_tao,
                },
                { label: "Số điện thoại", value: studentData.so_dien_thoai },
                {
                  label: "Số điện thoại gia đình",
                  value: studentData.dien_thoai_gia_dinh,
                },
                {
                  label: "Điện thoại cơ quan",
                  value: studentData.dien_thoai_CQ,
                },
                { label: "Email", value: studentData.email },
                {
                  label: "Khi cần báo tin cho ai",
                  value: studentData.khi_can_bao_tin_cho_ai,
                },
                {
                  label: "Nội trú",
                  value: studentData.noi_tru ? "Có" : "Không",
                },
                {
                  label: "Ngoại trú",
                  value: studentData.ngoai_tru ? "Có" : "Không",
                },
              ].map((item, index) =>
                item.value ? (
            ...
            <Grid maxWidth="" xs={12} sm={3} container spacing={2}>
              {[
                ...
                { label: "Cấp bậc", value: militaryData.cap_bac },
                {
                  ...
                  value: militaryData.trinh_do_van_hoa,
                },
                { label: "Nơi ở hiện nay", value: militaryData.noi_o_hien_nay },
                {
                  label: "Đơn vị cử đi học",
                  value: militaryData.don_vi_cu_di_hoc,
                },
      ...
      <Dialog maxWidth="xl" open={open} onClose={handleClose}>
        <DialogTitle>
          {editIndex !== null
            ? `Chỉnh sửa học viên: ${
                studentData.ho_dem + " " + studentData.ten
              }`
            : `Thêm học viên`}
        </DialogTitle>
        <DialogContent>
          <Grid container spacing={2}>
            ...
            {[
              ...
              { label: "Quê quán", key: "que_quan", required: true },
            ...
            {[
              ...
              },
              {
                ...
                key: "doi_tuong_id",
                type: "api",
                options: danhSachDoiTuongQL,
                optionLabel: "chi_tiet_doi_tuong",
                required: true,
              },
              {
                label: "Đang học",
              ...
              {
                label: "Trúng tuyển theo nguyện vọng",
                key: "trung_tuyen_theo_nguyen_vong",
              },
      ...
      <Dialog
        maxWidth="xl"
        open={openMilitaryPopup}
        onClose={handleCloseMiPopup}
      ...
      >
        ...
        <DialogContent>
          <Grid container spacing={2}>
            {[
              ...
              const isDateField = ["ngay_nhap_ngu", "ngay_nhan_luong"].includes(
                key
              );
        ...
        <DialogActions>
          <Button onClick={handleCloseMiPopup} color="secondary">
            Hủy
          </Button>
          <Button onClick={handleSaveMilitary} color="primary">
            Lưu
          </Button>
        </DialogActions>
      </Dialog>
